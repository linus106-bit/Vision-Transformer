...
class PositionalEmbedding(nn.Module):
    def __init__(self):
        ...
        # TODO : Find the difference of the cls token & pos emb
        self.x_class = nn.Parameter(torch.randn(b,1,d))
        self.pos_emb = nn.Parameter(torch.randn(1,n*n+1,d))
    ...
    #   pos_emb = nn.Parameter(torch.randn(b,n*n+1,d)).to(device)
      with_class_pos = class_patch + self.pos_emb # 이게 맞나? 그냥 더하는게?
...
# D = patchdata.Flattened2Dpatches(dataname='cifar10', img_size=w, patch_size=p,
#                                     batch_size=b)
# train_loader, val_loader, test_loader = D.patchdata()
